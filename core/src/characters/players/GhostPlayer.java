...
public class GhostPlayer extends GameObject {
    ...
    private void createAnimationController(){
        ...
        float animationTimes[] = null;

        if(this.characterType == IDs.CharacterType.SOLDIER) {
            animationsIds = Converter.soldierSkinToAnimationsIds(this.skin);
            animationFrames = new int[]{2, 4, 5};
            animationTimes = new float[]{0.3f, 0.2f, 0.15f};
        } else if(this.characterType == IDs.CharacterType.CLOWN) {
            animationsIds = Converter.clownSkinToAnimationsIds(this.skin);
            animationFrames = new int[]{5, 4, 9};
            animationTimes = new float[]{0.15f, 0.2f, 0.15f};
        } else if(this.characterType == IDs.CharacterType.KNIGHT) {
            animationsIds = Converter.knightSkinToAnimationsIds(this.skin);
            animationFrames = new int[]{5, 4, 5};
            animationTimes = new float[]{0.15f, 0.2f, 0.15f};
        } else if(this.characterType == IDs.CharacterType.PIRATE) {
            animationsIds = Converter.pirateSkinToAnimationsIds(this.skin);
            animationFrames = new int[]{2, 4, 12};
            animationTimes = new float[]{0.3f, 0.2f, 0.1f};
        }

        int ids[]                   = animationsIds;
        int frames[]                = animationFrames;
        float times[]               = animationTimes;
