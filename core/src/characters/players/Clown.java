...
public class Clown extends Player{
    ...
    public Clown(Vector2 position, int skin) {
        super(Converter.clownSkinToIdle(skin), position, new Vector2(Player.PLAYER_WH, Player.PLAYER_WH));
        this.skin = skin;
        super.weapon = Converter.idToSprite(IDs.HAMMER);
    ...
    private void createAnimationController(){
        int ids[]                   = Converter.clownSkinToAnimationsIds(this.skin);
        int frames[]                = {5, 4, 9};
    ...
    public void shoot(List<Bullet> bulletList) {
        if(super.timePassedToShoot >= super.SHOOT_DELAY) {
            if (super.isCanShoot()) {
                ...
                Bullet gb = new SpinningBullet(IDs.HAMMER, this);
                bulletList.add(gb);
