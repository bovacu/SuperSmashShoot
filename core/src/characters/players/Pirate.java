...
public class Pirate extends Player {
    ...
    public Pirate(Vector2 position, int skin) {
        super(Converter.pirateSkinToIdle(skin), position, new Vector2(Player.PLAYER_WH, Player.PLAYER_WH));
        this.skin = skin;
        super.weapon = Converter.idToSprite(IDs.PISTOL);
    ...
    private void createAnimationController(){
        int ids[]                   = Converter.pirateSkinToAnimationsIds(this.skin);
        int frames[]                = {2, 4, 12};
        float times[]               = {0.3f, 0.2f, 0.1f};
